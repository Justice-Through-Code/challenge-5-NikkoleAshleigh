...
def day_of_week():
    ...
    for single_day in range(len(days)):
        print(days[single_day])
# day_of_week()
...
def favorite_activities():
    ...
    for single_day in days:
        fav_thing = input(f'What is your favorite thing to do on a {single_day}? ')
    ...
    # AFTER your loop, print out the list to check if it got populated correctly.
        ...
        fav_activities.append(fav_thing)
...
# ----------------
    # can make a call in for loop on either function because both lists are the SAME length, 
    # range of the length of the list is what is considered when making that call WITH [i]
# -------------------
...
# ----------------
#     This program was hard enough with these smaller tasks, it definitely would not have
#       felt doable without breaking it down into steps! It could have been made a bit more clear.
# --------------
...
def temp_by_day():
    ...
    for single_day in days:
        temp_input = int(input(f'What is the temperature on {single_day}? '))
        if temp_input < 50:
